                ...
                using (StreamReader reader = new StreamReader(response.GetResponseStream()))
                {
                    ...
                    foreach (AStarBoardState haven in safeHavens)
                    {
                        ...
                        while(it.m_projectedPlayerTile.X == m_playerTile.X && it.m_projectedPlayerTile.Y == m_playerTile.Y)
                        {
                            ...
                            if(!it.m_projectedPlayerTile.isSafeUntil(it.m_cost + 5)) //TODO: this is broken. We don't need to look this far ahead
                            {
                        ...
                        if (m_playerTile.isSafeUntil(haven.m_cost + 5)  && isSuperSafeRoute && playerBombs < availableBombs && !m_parsed.bombMap.ContainsKey(m_playerTile.X + "," + m_playerTile.Y))//TODO: calculate how many bombs you have and drop that many.
                        {
                            canBomb = true;
                        }
                    }
